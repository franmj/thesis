...
\end{figure}
\subsubsection{Comparative docking approach}
...
\subsection{Targeted cancer therapies}\label{targetd therapies}
...
\par As we have shown over the Introduction physical interactions between small molecule and proteins govern many of the cellular functions \ref{ligand_intect}. Such is the importance that the modulation of the protein function by a small molecules has been used by to treat multiple conditions \ref{drug_discovery}. In fact, the discovery and pharmacological development of antibiotics for the treatment of infectious diseases such as dramatically improved our lifespan \ref{mtb_research}. More recently, the emergence of targeted cancer therapies also transformed the landscape of cancer treatment, moving from the traditionally cytotoxic chemotherapy to more precise targeted therapies \ref{targetd therapies}. Research are partly thanks to the development of methods to experimentally determine the 3D structure of proteins \ref{structure determination}. Furthermore, \textit{in-silico} methods have contributed to characterize protein and ligand interactions, with the added value of providing new predicted interactions \ref{prediction ligand-target}. The ability of computational methods to predict the protein targets of a given small molecule has significantly improved over the last years. This is partly cause by the emergence of databases such as Chembl or pubchem, pDB that gather many information structural and therapeutical infromation that can be used by computational models to infer new methods. The growth of data will increase the accuracy of the methods. However, this methods need to adapt to the requieremtnts of drug discovery processes where the structure is nedede. Most of the current methods do not allow large scale structural screening so we developed a method that i) leverages infromation to predict . ii) cann be applicable proteime. 

It was applied to TB. 

Cancer thing.   

%owever, in spite of the tremendous efforts, we do not completely understand yet all the processes involved in the binding of small molecules into their protein targets. Therefore, we are unable to accurately determine all the binding targets of a small molecule. Despite of the great research advances in the ligand-protein interaction field we do not completely understand yet all the processes involved in the binding of small molecules into their protein targets.

\par Despite of the big efforts there are still nnumerous challengues to address in the small molecule. We are still far from understanding how small molecules , so therefore we cannot yet predict all the possible targets of a small molecule. To help in this topic, we have deveo a method that 1) bla bla 2) bala. 
Application in TB which 3) 5) 
