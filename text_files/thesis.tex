...
\par The search for molecules with activity against MTB is employing many approaches in parallel including high throughput screening and computational methods. Such screenings aim to find new molecular entities that could lead to the development of new antibacterial treatments. One of these HTS approaches is the \textit{cell based phenotypic screening}, which represents a powerful approach to identify anti-mycobacterial compounds and elucidate novel targets \cite{Manjunatha2015}. Some phenotypic screenings are then combined with toxicity assays to find those compounds with high anti-tubercular activity and good PK profile \cite{Ballell2013}. Other HTS approaches aim to identy highly potent molecules against a essential MTB target \cite{Park2015, Arora2014}.  Computational methods are essential in the analysis of the vast amount of data generated by HTS and provide a very powerful tool to identity those candidate molecules that could be prioritized for further development. 
\subsubsection{In-silico approaches in TB}
Similarly to many other diseases, CADD methods play a substantial role in the Tuberculosis R$\&$ field. Uncountable \textit{in-silico} methods have been published over the last decade \cite{Ekins2013}, each of them applies different strategies to solve a specific bio-medical question. However, all of them pursue the very same goal: fueling drug discovery against TB. 
Table X recapitule some  an example of the multiple approaches and goals of in-silico methods in the research against Tuberculosis Expandir. 
